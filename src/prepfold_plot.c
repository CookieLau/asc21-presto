...
/* Make the beautiful 1 page prepfold output */
{
    ...
    // lqq: here when the bestp < 0, it will occur -nan in DOF_corr
    // e.g. search->topo: {pow = 0, p1 = -2.193973653219234, p2 = 14.144518116409227, p3 = 0}}

    // The number of samples that can fit across one profile bin
    dt_per_bin = bestp / search->proflen / search->dt;
    // printf("[lqq] bestp:%lf search->proflen:%d search->dt:%lf\n", bestp, search->proflen, search->dt);
    ...
    // printf("[lqq] search->proflen:%d dt_per_bin:%f DOF_corr(dt_per_bin):%lf dofeff:%f\n", search->proflen, dt_per_bin, DOF_corr(dt_per_bin), dofeff); 
