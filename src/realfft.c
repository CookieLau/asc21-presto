...
#define INDIVIDUAL 1 // [lqq] define
...
int main(int argc, char *argv[])
{
...
#if INDIVIDUAL // [lqq] define
    /* Open and check data files */
    for (jj = 0; jj < numfiles; jj++) {
        datfile = fopen_multifile(1, datfilenms + jj, "r", 0);
        maxfilelen = datfile->filelens[0];
        printf("   %d:  '%s'\n", jj + 1, datfile->filenames[0]);
        ...
        if ((numdata > MAXREALFFT || cmd->diskfftP) && !cmd->memfftP) {
            ...
            printf("[lqq] perform two-pass out-of-cofe FFT functions\n");
            if (isign == -1) {
            ...
            printf("[lqq] perform invididual standard FFT function\n");
            outfile = fopen_multifile(1, outfilenms + jj, "w", maxfilelen);
            if (isign == -1) {
    ...
    if ((numdata > MAXREALFFT || cmd->diskfftP) && !cmd->memfftP) {
        ...
        /* Perform standard FFT for real functions  */
        printf("[lqq] perform standard FFT function\n");
