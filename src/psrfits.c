...
int get_PSRFITS_rawblock(const float *fdata, struct spectra_info *s, int *padding)
...
void get_PSRFITS_subint(const float *fdata, unsigned char *cdata, struct spectra_info *s)
{
    unsigned char *ctmp = cdata;
    int ii, status = 0, anynull;
    int numtoread = s->samples_per_subint;
    if (s->num_channels & 0xf) {
        printf("[lqq] fuck num_chan:%d\n", s->num_channels);
    }
    ...
    // 将小于 8 bit 的 sample 打包成至少 8 bit 后面读取方便
    if (likely(s->bits_per_sample >= 8)) {
        goto skip_packed;
        ...
        if (s->use_poln > 0 || ((s->num_polns > 2) && !sum_polns)) {
            ...
            if (s->bits_per_sample == 16) {
                ...
                for (ii = 0; ii < s->spectra_per_subint; ii++) {
                    ...
                    for (jj = 0; jj < s->num_channels; jj++)
                        fptr[jj] =
                            (((float) sptr[jj] - s->zero_offset) * scales[idx + jj] + offsets[idx + jj]) * weights[jj];
                }
                ...
                for (ii = 0; ii < s->spectra_per_subint; ii++) {
                    ...
                    for (jj = 0; jj < s->num_channels; jj++)
                        fptr[jj] =
                            (((float) ftptr[jj] - s->zero_offset) * scales[idx + jj] + offsets[idx + jj]) * weights[jj];
                }
            } else {
                #ifdef _OPENMP
                #pragma omp parallel for shared(s,cdata,fdata,scales,offsets,weights)
                ...
                for (ii = 0; ii < s->spectra_per_subint; ii++) {
                    ...
                    for (jj = 0; jj < s->num_channels; jj++)
                        fptr[jj] =
                            (((float) cptr[jj] - s->zero_offset) * scales[idx + jj] + offsets[idx + jj]) * weights[jj];
                }
            }
        } else if (sum_polns) { // sum the polns if there are 2 by default
            const int idx = s->num_channels;
            // printf("[lqq] no use_poln\n");
            int apply_type = (s->apply_offset << 2) | (s->apply_scale << 1) | s->apply_weight;
            /*  apply_type is the bitwise of three apply
                0. 000 remove offset, scales and weight
                1. 001 remove offset, scales 
                2. 010 remove offset, weight
                3. 011 remove offset
                4. 100 remove scales, weight
                5. 101 remove scales
                6. 110 remove weight
                7. 111 no remove
            */
            switch (apply_type) {
                case 0:
                    ...
                    if (likely(s->bits_per_sample <= 8)) {
                        ...
                        #endif
                        for (ii = 0; ii < s->spectra_per_subint; ii++) { // 2400 1024
                            int jj;
                            float *fptr = fdata + ii * s->num_channels;
                            const unsigned char *cptr = cdata + ii * s->samples_per_spectra;
                            #pragma simd
                            #pragma vector aligned
                            for (jj = 0; jj < s->num_channels; jj++) { // 4096 96
                                fptr[jj] = ((float) cptr[jj] - s->zero_offset) + ((float) cptr[jj + idx] - s->zero_offset);
                            }
                        }
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = ((float) sptr[jj] - s->zero_offset) + ((float) sptr[jj + idx] - s->zero_offset);
                            }
                        }
                    } else if (s->bits_per_sample == 32) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = ((float) ftptr[jj] - s->zero_offset) + ((float) ftptr[jj + idx] - s->zero_offset);
                            }
                        }
                    } else { 
                        printf("[lqq] Error in bits_per_sample: %d not a good number\n", s->bits_per_sample);
                        exit(-1);
                    }
                    break;
                case 1:
                    // 001 remove offset, scales 
                    if (likely(s->bits_per_sample <= 8)) {
                        #ifdef _OPENMP
                        #pragma omp parallel for shared(s,cdata,fdata,weights)
                        #endif
                        for (ii = 0; ii < s->spectra_per_subint; ii++) { // 2400 1024
                            int jj;
                            float *fptr = fdata + ii * s->num_channels;
                            ...
                            #pragma vector aligned
                            for (jj = 0; jj < s->num_channels; jj++) { // 4096 96
                                fptr[jj] = ((float) cptr[jj] - s->zero_offset + (float) cptr[jj + idx] - s->zero_offset) * weights[jj];
                            }
                        }
                    } else if (s->bits_per_sample == 16) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            const short *sptr =
                                (short *) cdata + ii * s->samples_per_spectra;
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = ((float) sptr[jj] - s->zero_offset + (float) sptr[jj + idx] - s->zero_offset) * weights[jj];
                            }
                        }
                    } else if (s->bits_per_sample == 32) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            const float *ftptr =
                                (float *) cdata + ii * s->samples_per_spectra;
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = ((float) ftptr[jj] - s->zero_offset + (float) ftptr[jj + idx] - s->zero_offset) * weights[jj];
                            }
                        }
                    } else { 
                        printf("[lqq] Error in bits_per_sample: %d not a good number\n", s->bits_per_sample);
                    ...
                    break;
                case 2:
                    // 010 remove offset, weight
                    if (likely(s->bits_per_sample <= 8)) {
                        #ifdef _OPENMP
                        #pragma omp parallel for shared(s,cdata,fdata,scales)
                        #endif
                        for (ii = 0; ii < s->spectra_per_subint; ii++) { // 2400 1024
                            int jj;
                            float *fptr = fdata + ii * s->num_channels;
                            ...
                            #pragma vector aligned
                            for (jj = 0; jj < s->num_channels; jj++) { // 4096 96
                                fptr[jj] = (((float) cptr[jj] - s->zero_offset) * scales[jj]) + (((float) cptr[jj + idx] - s->zero_offset) * scales[idx + jj]);
                            }
                        }
                    } else if (s->bits_per_sample == 16) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            const short *sptr =
                                (short *) cdata + ii * s->samples_per_spectra;
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = (((float) sptr[jj] - s->zero_offset) * scales[jj]) + (((float) sptr[jj + idx] - s->zero_offset) * scales[idx + jj]);
                            }
                        }
                    } else if (s->bits_per_sample == 32) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            const float *ftptr =
                                (float *) cdata + ii * s->samples_per_spectra;
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = (((float) ftptr[jj] - s->zero_offset) * scales[jj]) + (((float) ftptr[jj + idx] - s->zero_offset) * scales[idx + jj]);
                            }
                        }
                    } else { 
                        printf("[lqq] Error in bits_per_sample: %d not a good number\n", s->bits_per_sample);
                ...
                case 3:
                    ...
                    if (likely(s->bits_per_sample <= 8)) {
                        #ifdef _OPENMP
                        #pragma omp parallel for shared(s,cdata,fdata,scales,weights)
                        #endif
                        for (ii = 0; ii < s->spectra_per_subint; ii++) { // 2400 1024
                            int jj;
                            float *fptr = fdata + ii * s->num_channels;
                            ...
                            #pragma vector aligned
                            for (jj = 0; jj < s->num_channels; jj++) { // 4096 96
                                fptr[jj] = (((float) cptr[jj] - s->zero_offset) * scales[jj] + ((float) cptr[jj + idx] - s->zero_offset) * scales[idx + jj]) * weights[jj];
                            }
                        }
                    } else if (s->bits_per_sample == 16) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            const short *sptr =
                                (short *) cdata + ii * s->samples_per_spectra;
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = (((float) sptr[jj] - s->zero_offset) * scales[jj] + ((float) sptr[jj + idx] - s->zero_offset) * scales[idx + jj]) * weights[jj];
                            }
                        }
                    } else if (s->bits_per_sample == 32) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            const float *ftptr =
                                (float *) cdata + ii * s->samples_per_spectra;
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = (((float) ftptr[jj] - s->zero_offset) * scales[jj] + ((float) ftptr[jj + idx] - s->zero_offset) * scales[idx + jj]) * weights[jj];
                            }
                        }
                    } else { 
                        printf("[lqq] Error in bits_per_sample: %d not a good number\n", s->bits_per_sample);
                    ...
                    break;
                case 4:
                    // 100 remove scales, weight
                    if (likely(s->bits_per_sample <= 8)) {
                        #ifdef _OPENMP
                        #pragma omp parallel for shared(s,cdata,fdata,offsets)
                        #endif
                        for (ii = 0; ii < s->spectra_per_subint; ii++) { // 2400 1024
                            int jj;
                            float *fptr = fdata + ii * s->num_channels;
                            ...
                            #pragma vector aligned
                            for (jj = 0; jj < s->num_channels; jj++) { // 4096 96
                                fptr[jj] = (((float) cptr[jj] - s->zero_offset) + offsets[jj]) + (((float) cptr[jj + idx] - s->zero_offset) + offsets[idx + jj]);
                            }
                        }
                    } else if (s->bits_per_sample == 16) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            const short *sptr =
                                (short *) cdata + ii * s->samples_per_spectra;
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = (((float) sptr[jj] - s->zero_offset) + offsets[jj]) + (((float) sptr[jj + idx] - s->zero_offset) + offsets[idx + jj]);
                            }
                        }
                    } else if (s->bits_per_sample == 32) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            const float *ftptr =
                                (float *) cdata + ii * s->samples_per_spectra;
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = (((float) ftptr[jj] - s->zero_offset) + offsets[jj]) + (((float) ftptr[jj + idx] - s->zero_offset) + offsets[idx + jj]);
                            }
                        }
                    } else { 
                        printf("[lqq] Error in bits_per_sample: %d not a good number\n", s->bits_per_sample);
                ...
                case 5:
                    ...
                    if (likely(s->bits_per_sample <= 8)) {
                        #ifdef _OPENMP
                        #pragma omp parallel for shared(s,cdata,fdata,offsets,weights)
                        #endif
                        for (ii = 0; ii < s->spectra_per_subint; ii++) { // 2400 1024
                            int jj;
                            float *fptr = fdata + ii * s->num_channels;
                            ...
                            #pragma vector aligned
                            for (jj = 0; jj < s->num_channels; jj++) { // 4096 96
                                fptr[jj] = (((float) cptr[jj] - s->zero_offset)  + offsets[jj] + ((float) cptr[jj + idx] - s->zero_offset)  + offsets[idx + jj]) * weights[jj];
                            }
                        }
                    } else if (s->bits_per_sample == 16) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            const short *sptr =
                                (short *) cdata + ii * s->samples_per_spectra;
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = (((float) sptr[jj] - s->zero_offset)  + offsets[jj] + ((float) sptr[jj + idx] - s->zero_offset)  + offsets[idx + jj]) * weights[jj];
                            }
                        }
                    } else if (s->bits_per_sample == 32) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            const float *ftptr =
                                (float *) cdata + ii * s->samples_per_spectra;
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = (((float) ftptr[jj] - s->zero_offset)  + offsets[jj] + ((float) ftptr[jj + idx] - s->zero_offset)  + offsets[idx + jj]) * weights[jj];
                            }
                        }
                    } else { 
                        printf("[lqq] Error in bits_per_sample: %d not a good number\n", s->bits_per_sample);
                ...
                case 6:
                    ...
                    if (likely(s->bits_per_sample <= 8)) {
                        #ifdef _OPENMP
                        #pragma omp parallel for shared(s,cdata,fdata,scales,offsets)
                        #endif
                        for (ii = 0; ii < s->spectra_per_subint; ii++) { // 2400 1024
                            int jj;
                            float *fptr = fdata + ii * s->num_channels;
                            ...
                            #pragma vector aligned
                            for (jj = 0; jj < s->num_channels; jj++) { // 4096 96
                                fptr[jj] = (((float) cptr[jj] - s->zero_offset) * scales[jj] + offsets[jj]) + (((float) cptr[jj + idx] - s->zero_offset) * scales[idx + jj] + offsets[idx + jj]);
                            }
                        }
                    } else if (s->bits_per_sample == 16) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            const short *sptr =
                                (short *) cdata + ii * s->samples_per_spectra;
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = (((float) sptr[jj] - s->zero_offset) * scales[jj] + offsets[jj]) + (((float) sptr[jj + idx] - s->zero_offset) * scales[idx + jj] + offsets[idx + jj]);
                            }
                        }
                    } else if (s->bits_per_sample == 32) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            const float *ftptr =
                                (float *) cdata + ii * s->samples_per_spectra;
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = (((float) ftptr[jj] - s->zero_offset) * scales[jj] + offsets[jj]) + (((float) ftptr[jj + idx] - s->zero_offset) * scales[idx + jj] + offsets[idx + jj]);
                            }
                        }
                    } else { 
                        printf("[lqq] Error in bits_per_sample: %d not a good number\n", s->bits_per_sample);
                ...
                case 7:
                    if (likely(s->bits_per_sample <= 8)) {
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) { // 2400 1024
                            ...
                            #pragma vector aligned
                            for (jj = 0; jj < s->num_channels; jj++) { // 4096 96
                                fptr[jj] = (((float) cptr[jj] - s->zero_offset) * scales[jj] + offsets[jj] + ((float) cptr[jj + idx] - s->zero_offset) * scales[idx + jj] + offsets[idx + jj]) * weights[jj];
                            }
                        }
                    } else if (s->bits_per_sample == 16) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            const short *sptr =
                                (short *) cdata + ii * s->samples_per_spectra;
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = (((float) sptr[jj] - s->zero_offset) * scales[jj] + offsets[jj] + ((float) sptr[jj + idx] - s->zero_offset) * scales[idx + jj] + offsets[idx + jj]) * weights[jj];
                            }
                        }
                    } else if (s->bits_per_sample == 32) {
                        #ifdef _OPENMP
                        ...
                        for (ii = 0; ii < s->spectra_per_subint; ii++) {
                            ...
                            const float *ftptr =
                                (float *) cdata + ii * s->samples_per_spectra;
                            for (jj = 0; jj < s->num_channels; jj++) {
                                fptr[jj] = (((float) ftptr[jj] - s->zero_offset) * scales[jj] + offsets[jj] + ((float) ftptr[jj + idx] - s->zero_offset) * scales[idx + jj] + offsets[idx + jj]) * weights[jj];
                            }
                        }
                    } else { 
                        printf("[lqq] Error in bits_per_sample: %d not a good number\n", s->bits_per_sample);
                    ...
                    break;
                default:
                    printf("Error in psrfits, apply_offset=%d apply_scales=%d apply_weights=%d\n", s->apply_offset, s->apply_scale, s->apply_weight);
            }
        }
