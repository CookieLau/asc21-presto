...
fcomplex *gen_z_response(double roffset, int numbetween, double z, int numkern)
  ...
  /*       contain.                                                    */
{
    printf("[lqq] use gen_z_response_avx512 %lf %d %lf %d\n", roffset, numbetween, z, numkern);

    int ii, signz, numkernby2;
    double absz, zd, r, xx, yy, zz, startr, startroffset;
    ...
    // printf("\n[lqq] start for loop\n");
    for (ii = 0, r = startr; ii < numkern; ii++, r += delta) {
        // printf("[lqq] ii:%d numkern:%d r:%lf delta:%lf\n", ii, numkern, r, delta);
        yy = r * zd;
        zz = yy + z * zd;
        xx = pibyz * r * r;
        c = cos(xx);
        s = sin(xx);
        // printf("[lqq] xx:%lf yy:%lf zz:%lf c:%lf s:%lf\n", xx, yy, zz, c, s);
