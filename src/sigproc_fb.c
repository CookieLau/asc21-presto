...
/* This routine converts SIGPROC filterbank-format data into PRESTO format */
{
    ...
    if (s->bits_per_sample == 32) {
        ...
        for (spec_ct = 0; spec_ct < numread; spec_ct++) {
            ...
            #else
            const __m256 _one = _mm256_set1_ps(1E0);
            const __m256i _permute_idx = _mm256_setr_epi32(7, 6, 5, 4, 3, 2, 1, 0);
            int startidx = s->num_channels / 8 - 1 + offset / 8;
            for (ii = 0, jj = startidx; ii < s->num_channels; ii += 8, outdataptr += 8, jj--) {
                c = chardata[jj];
                _mm256_store_ps(outdataptr, _mm256_permutevar8x32_ps(_mm256_maskz_mov_ps(chardata[jj], _one), _permute_idx));
            }
            #endif
        }
    } else {
