...
def batch_dedisperse_n_fft(name, prep, prepcmd, real_list, accel_list):
    ...
    buffer = []
    prep = "taskset -c {} {}".format(ccpu, prep)
    print(prep)
    output = getoutput(prep)
    buffer.append(output)
    prepcmd = "taskset -c {} {}".format(ccpu, prepcmd)
    print(prepcmd)
...
def batch_prepfold(fold):
    ...
    ccpu = int(name.split("-")[1]) % multiprocessing.cpu_count()
    st = time.time()
    fold = "taskset -c {} {}".format(ccpu, fold)
    print(fold)
